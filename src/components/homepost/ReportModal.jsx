...
function ReportModal({ userId, postId, closeModal }) {
  ...
  const formik = useFormik({
    initialValues: {
      reason: "",
      reasonType: "", // Add reasonType field
    },
    validationSchema: Yup.object({
      ...
      reasonType: Yup.string()
        .required("Please select a reason type"), // Ensure reasonType is selected
    }),
    onSubmit: async () => {
      const { reason, reasonType } = formik.values;
  
      try {
        const response = await reportPost({ userId, postId, reason, reasonType });
            ...
            {post?.imgUrl && (
              <div className="mb-4">
                <img
                  src={post.imgUrl}
                  alt="Post Image"
                  className="w-60 h-auto max-h-60 object-cover mx-auto rounded"
                />
              </div>
            )}

            <form onSubmit={formik.handleSubmit} className="max-w-md mx-auto">
              {/* Dropdown for selecting reason type */}
              <div className="relative z-0 w-full mb-5 group">
                ...
                >
                  <option value="" label="Select reason type" />
                  <option value="spam" label="Spam" />
                  <option value="abuse" label="Abusive Content" />
              ...
              <div className="relative z-0 w-full mb-5 group">
                ...
                {formik.touched.reason && formik.errors.reason && (
                  <p className="text-red-600 text-xs">{formik.errors.reason}</p>
                )}
              </div>
